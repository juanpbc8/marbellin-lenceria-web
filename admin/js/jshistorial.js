let historialCompras = JSON.parse(localStorage.getItem("historialCompras")) || [];
    ...
    function mostrarHistorial() {
      ...
      historialCompras.forEach((boleta, index) => {
        const estadoEntregado = boleta.estado && boleta.estado.toLowerCase() === 'entregado';
    ...
    function editarEstado(index) {
      Swal.fire({
        ...
        inputValue: historialCompras[index].estado ? historialCompras[index].estado.toLowerCase() : 'pendiente',
    ...
    function eliminarHistorial() {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          historialCompras = [];
          localStorage.setItem("historialCompras", JSON.stringify(historialCompras));
          localStorage.setItem("numeroFactura", "0"); // Reiniciar número de factura si usas
          mostrarHistorial();
